...
function removeOldSearchResults() {
  ...
  chrome.storage.local.get({'storedArray': []}, function(data) {
    ...
    oldData.forEach( (something)=> {
      ...
      } if we could make the extension scroll and wait when all images appear do it's work
    });
    */
    ...
    for (var i=0; i<liCards.length; i++){
      ...
      if (oldData.includes(boom) == true){
            data_toremove.length;
            data_toremove.push(liCards[i]);
        }
      if (data_toremove.length != 0){
        data_toremove.forEach( (removeitem)=>{
          removeitem.remove();
        });
      }
      if (oldData.includes(boom) == false){
        ...
        //setTimeout(function(){ liCards[i].style.display = "none"; }, 200);
